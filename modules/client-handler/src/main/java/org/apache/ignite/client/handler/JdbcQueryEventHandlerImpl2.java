 ...
 */
public class JdbcQueryEventHandlerImpl2 implements JdbcQueryEventHandler {
     ...
     */
    public JdbcQueryEventHandlerImpl2(QueryProcessor processor) {
    ...
    @Override public JdbcQueryExecuteResult query(JdbcQueryExecuteRequest req) {
        ...
        try {
            for (SqlCursor<List<?>> cur : cursors) {
                if (!supportCursorType(cur.getQueryType())) {
                    results.add(new JdbcQuerySingleResult(UNSUPPORTED_OPERATION,
                ...
                results.add(new JdbcQuerySingleResult(cursorId, cur.getQueryType().id()));
            }
        } catch (Exception ex) {
            return new JdbcQueryExecuteResult(JdbcResponse.STATUS_FAILED,
    ...
    private boolean supportCursorType(SqlQueryType type) {
        return type != SqlQueryType.FRAGMENT && type != SqlQueryType.EXPLAIN;
    }
