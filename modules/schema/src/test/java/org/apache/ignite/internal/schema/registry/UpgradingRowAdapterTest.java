...
public class UpgradingRowAdapterTest {
    ...
    public void testVariousColumnTypes() {
        ...
        // Validate row.
        validateRow(values, new SchemaRegistryImpl(v -> v == 1 ? schema : schema2, () -> 1, schema), row);

        // Validate upgraded row.
        values.add(addedColumnIndex, null);

        validateRow(values, new SchemaRegistryImpl(v -> v == 1 ? schema : schema2, () -> 1, schema2), row);
    }
