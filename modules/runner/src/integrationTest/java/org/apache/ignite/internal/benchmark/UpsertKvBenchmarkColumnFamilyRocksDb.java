...
public class UpsertKvBenchmarkColumnFamilyRocksDb {
    static {
        RocksDB.loadLibrary();
    }
    ...
    ThreadLocal<ByteBuffer> key = ThreadLocal.withInitial(() -> ByteBuffer.allocate(Integer.BYTES));
    ...
    ThreadLocal<ByteBuffer> val = ThreadLocal.withInitial(() -> {
        ...
        ByteBuffer val = ByteBuffer.allocate(data.length);
        val.put(data);
    ...
    public void upsert() throws RocksDBException {
        ...
        ByteBuffer tmp = val.get();
        ByteBuffer valBuf = ByteBuffer.allocate(tmp.array().length);
        valBuf.put(tmp.array());

        WriteBatch writeBatch = new WriteBatch();
        writeBatch.put(dataCf.cfHandle, keyBuf.array(), valBuf.array());
        rocksDB.write(writeOptions, writeBatch);
        writeBatch.close();

//        rocksDB.put(dataCf.cfHandle, writeOptions, keyBuf.array(), valBuf.array());
    }
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(".*" + UpsertKvBenchmarkColumnFamilyRocksDb.class.getSimpleName() + ".*")
    ...
    private static ColumnFamilyOptions defaultCfOptions() {
        return new ColumnFamilyOptions()
                ...
                .setWriteBufferSize(8L * 1024 * 1024 * 1024).setMemTableConfig(new SkipListMemTableConfig());
        //return new ColumnFamilyOptions().setMemTableConfig(new SkipListMemTableConfig());
    }
