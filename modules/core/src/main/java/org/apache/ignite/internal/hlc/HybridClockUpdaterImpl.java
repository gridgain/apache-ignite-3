 ...
 */
public class HybridClockUpdaterImpl implements HybridClock {
    private final IgniteLogger log = Loggers.forClass(HybridClockUpdaterImpl.class);

    /**
     * Var handle for {@link #latestTime}.
     */
    private static final AtomicLongFieldUpdater<HybridClockUpdaterImpl> LATEST_TIME = AtomicLongFieldUpdater.newUpdater(HybridClockUpdaterImpl.class,
            "latestTime");
     ...
     */
    public HybridClockUpdaterImpl() {
        this.latestTime = currentTime();
    }

    /**
     * System current time in milliseconds shifting left to free insignificant bytes. This method is marked with a public modifier to mock
     * in tests because there is no way to mock currentTimeMillis.
     *
